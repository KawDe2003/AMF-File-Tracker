          ...
          <div style={{ padding: '1.5rem' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {users.length === 0 ? (
                ...
                <div key={u.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-md)' }}>
                  <div>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                    ...
                    <span style={{ fontSize: '0.8rem', color: 'var(--text-muted)' }}>{u.department?.name || 'No Dept'}</span>
                  </div>
                </div>
              ))}
            </div>
             ...
             <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
              {departments.length === 0 ? (
                ...
                <div key={d.id} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '1rem', border: '1px solid var(--border-color)', borderRadius: 'var(--radius-md)' }}>
                  ...
                  <span className="badge" style={{ backgroundColor: 'var(--bg-color)', color: 'var(--text-muted)' }}>{d.id.substring(0, 5)}</span>
                </div>
              ))}
            </div>
          </div>
